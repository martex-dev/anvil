...
describe('toast store', () => {
	...
	it('never folds toasts with an action, so each Undo undoes its own event', () => {
		const first = vi.fn();
		const second = vi.fn();
		toast.info('Conversation cleared', undefined, { label: 'Undo', run: first });
		toast.info('Conversation cleared', undefined, { label: 'Undo', run: second });
		const toasts = useToastStore.getState().toasts;
		expect(toasts).toHaveLength(2);
		toasts[1]?.action?.run();
		expect(second).toHaveBeenCalled();
		expect(first).not.toHaveBeenCalled();
		// An action toast also stays up longer, like an error.
		expect(toasts[1]?.durationMs).toBe(8000);
	});
});
