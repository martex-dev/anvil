...
| `data`               | Parses CSV/TSV/JSON(L) in a worker thread (ADR-014). Parquet/Feather/Excel go through the user's Python (polars, then pandas). Results are cached; filter, sort, pages and column stats are served from the cache. |
